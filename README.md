...
> **Prerequisites:** All steps in [Setup Instructions](#setup-instructions) must be complete and the backend `.venv` must exist before packaging.

To produce a self-contained Windows NSIS installer (`.exe`), run the following from the repository root:
...
This command:
1. Runs `npm run build` in `frontend/` to produce the optimised Vite bundle.
2. Runs `npm run dist` in `electron/` which:
   - Compiles TypeScript (`tsc`).
   - Invokes `electron-builder --win` to package everything into an ASAR archive.
   - Bundles the entire `backend/` folder into `resources/backend/` inside the package (excluding `__pycache__` and `.pyc` files).
   - Generates an NSIS installer with the settings below.

**Output:**
```
electron/dist/LocalGravity Setup 0.1.0.exe
```

**NSIS installer settings:**
| Setting | Value |
|---|---|
| One-click install | Disabled — user steps through wizard |
...
When the packaged app is launched (`NODE_ENV !== 'development'`), the following changes take effect automatically:

| Feature | Development | Production |
|---|---|---|
| Swagger UI (`/docs`) | Available at `http://127.0.0.1:8000/docs` | **Disabled** — returns 404 |
| Electron DevTools | Opened detached on startup | **Never opened** |
| Backend spawn | Started manually via `npm run dev:backend` | **Auto-spawned** from `resources/backend/` using bundled `.venv` Python |
| Backend log level | `info` (verbose) | `warning` (minimal noise) |
| Debug logging | Full request/response tracing | **Off** — only warnings and errors |
| Source maps | Included | **Excluded** from ASAR (`!dist/**/*.map`) |

To verify production mode locally without building the installer:

```bash
# Build both layers first
npm run build

# Launch Electron in prod mode
cross-env NODE_ENV=production electron electron/dist/main.js
```

The backend will not auto-start in this mode unless `resources/backend/` exists, so point `LOCALGRAVITY_BACKEND_URL` to a running instance or start it manually.
